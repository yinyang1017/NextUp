...
export default class AutoCompleteInput extends React.Component {
  ...
  fetchAddressForLocation = location => {
    ...
    axios
      .get(`${REVRSE_GEO_CODE_URL}?key=${this.props.apiKey}&latlng=${latitude},${longitude}&result_type=administrative_area_level_2`, {
      ...
      .then(({ data }) => {
        ...
        if (results.length > 0) {
          ...
          if (address_components.length > 0) {
            this.setState({ text: address_components[0].long_name });
          }
        }
      });
  };
