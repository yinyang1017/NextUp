...
export const RecentGames = ({title = 'Recent Games'}) => {
  return (
    <>
      <View style={styles.upcomingGamesParent}>
        <Text style={[styles.myTeams, styles.vamTypo1]}>{title}</Text>
      ...
      <ScrollView
        ...
        {[...new Array(5)].map((item, index) => {
          return (
            <Pressable
              style={styles.subtractShadowBox}
              key={index}
            //   onPress={() => navigation.navigate('CoUpGame')}
              >
              <Image
                style={[styles.subtractIcon, styles.iconLayout1]}
                resizeMode="cover"
